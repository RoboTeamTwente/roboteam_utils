...
/* In this test class we test the different projection methods in different line classes: Line and LineSegment. For these methods we want to cover 3 type of lines: vertical line
 * (does not change in x direction), horizontal line (does not change in y direction), arbitrary other line (does change in both x and y direction). This is to test cases where
 * there could be a possibility of division by zero. Furthermore we test two different cases, where the point is on the Line interpretation and where the point is not on the Line
 * interpretation. As test method we use equivalence partitioning, based on the relative position of the Line interpretation projection: t < 0, 0 <= t <= 1, 1 < t. We extend these
 * test cases with boundary value analysis, but for t is (minus) infinity we use a very small and very large number (we expect intersections to fail for FLT_MAX and -FLT_MAX). Also
 * we create a 2 test case where we do a projection of a point on itself as LineSegment and a projection of a point on another point which is a LineSegment.
 */
TEST(LineProjectionTests, NotOnLine1) {
    ...
    Line line(start, end); LineSegment lineSegment(start, end);
...
TEST(LineProjectionTests, OnLine7) {
    ...
    Vector2 start(-5.0, 1e9 + EPSILON), end(-5.0, 1e9 - EPSILON), point(-5.0, -1e9);
    Line line(start, end); LineSegment lineSegment(start, end);
    Vector2 expected = {-5.0, -1e9};
    Vector2 actual = line.project(point);
    EXPECT_EQ(expected, actual);
    expected = {-5.0, 1e9 - EPSILON};
    actual = lineSegment.project(point);
    EXPECT_EQ(expected, actual);
}
