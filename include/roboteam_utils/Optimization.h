 ...
 * it's also slower.
 */
double optimizeLinear(double from,                             // The low end of the range
...
Vector2 optimizeVector(const Vector2 &center, double minXDev, double maxXDev, double minYDev, double maxYDev, double step, int refinement,
                       std::function<double(const Vector2 &)> scoreFunc);

Vector2 sampleForVector(const Vector2 &center, double minXDev, double maxXDev, double minYDev, double maxYDev, int samples, std::function<double(const Vector2 &)> scoreFunc);
...
class GradientDescent {
   public:
    ...
    GradientDescent(ScoreFunction scorer, Data data, Data initialSteps, std::array<std::pair<Num, Num>, N> limits)
        : scorer(scorer), data(data), steps(initialSteps), limits(limits), lastScore(-99999), justFlipped(true), iterationCount(0) {}
    ...
    Num getLastScore() const { return lastScore; }

    Num getLastDivergence() const { return lastDivergence; }

    size_t getIterationCount() const { return iterationCount; }

    Data getCurrentValues() const { return data; }
