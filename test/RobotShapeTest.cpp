...
TEST(RobotShapeTest,constructors){
    ...
    EXPECT_EQ(shape.centerOfKickerPos(),Vector2(0.05,0));
    LineSegment kicker = shape.kicker();
    double diff = sqrt(0.1*0.1-0.05*0.05);

    //Thesse are very crucial that they are in the right order,
    //as many of the internal calculations in the class rely on these being in exactly this order.
    EXPECT_EQ(kicker.start,Vector2(0.05,-diff));
    EXPECT_EQ(kicker.end,Vector2(0.05,diff));

    //Check if rotation is applied correctly
    RobotShape shape2(Vector2(0,0),0.05,0.1,Angle(1.0));
    EXPECT_EQ(shape2.pos(),Vector2(0,0));
    EXPECT_DOUBLE_EQ(shape2.angle().getValue(),1.0);
    EXPECT_DOUBLE_EQ(shape2.centerOfKickerPos().x,Vector2(0.05,0).rotate(1.0).x);
    EXPECT_DOUBLE_EQ(shape2.centerOfKickerPos().y,Vector2(0.05,0).rotate(1.0).y);
...
TEST(RobotShapeTest, lineIntersection){
    ...
    //EXPECT_EQ(shape.intersects(throughCornerAndFront).size(),1); //TODO: fix
