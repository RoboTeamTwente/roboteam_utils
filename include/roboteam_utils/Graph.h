...
class Graph {
   ...
   public:
     ...
     */
    void connect(const Vertex<T> &in, const Vertex<T> &out, const double cost) { adj[in.id].emplace_back(in, out, cost); }
    ...
    std::optional<std::list<Vertex<T>>> find(const Vertex<T> &start, const Vertex<T> &goal) const {
        ...
        while (!q.empty()) {
            Vertex<T> const &u = *std::min_element(q.begin(), q.end(), [costs](Vertex<T> a, Vertex<T> b) { return costs.at(a) < costs.at(b); });
     ...
     */
    std::optional<std::list<Vertex<T>>> astar(const Vertex<T> &start, const Vertex<T> &goal, std::function<double(const T &, const T &)> heuristic) const {
        ...
        while (!open.empty()) {
            Vertex<T> current = *std::min_element(open.begin(), open.end(), [&fscores](Vertex<T> a, Vertex<T> b) { return fscores.at(a) < fscores.at(b); });
