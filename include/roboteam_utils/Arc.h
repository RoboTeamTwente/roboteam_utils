 ...
 */
constexpr double ARC_MAX = 2 * M_PI - std::numeric_limits<float>::epsilon();
...
class Arc {
   public:
     ...
     */
    [[nodiscard]] std::pair<std::optional<Vector2>, std::optional<Vector2>> intersectionWithLine(Vector2 lineStart, Vector2 lineEnd) const;
