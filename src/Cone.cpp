...
bool Cone::IsWithinCone(Vector2 point) const noexcept { return IsWithinCone(point, 0.0); }
...
Vector2 Cone::ClosestPointOnSide(Vector2 point, Vector2 closeTo) const noexcept {
    ...
    if (option1Dist <= option2Dist) {
        if (IsWithinField(option1))
            return option1;
        else if (IsWithinField(option2))
            return option2;
        else
            return option1;
    } else {
        if (IsWithinField(option2))
            return option2;
        else if (IsWithinField(option1))
            return option1;
        else
            return option2;
    }
...
Vector2 Cone::ClosestPointOnSideTwoCones(Cone otherCone, Vector2 point, Vector2 closeTo) const noexcept {
    ...
    auto calc = [&](size_t idx) { return (intersections[idx] - closeTo).length(); };

    std::vector<double> costs{calc(0), calc(1), calc(2), calc(3)};

    return intersections.at(distance(costs.begin(), min_element(costs.begin(), costs.end())));
}
...
bool Cone::DoConesOverlap(Cone otherCone) const noexcept {
    return this->IsWithinCone(otherCone.side1 + otherCone.start) || this->IsWithinCone(otherCone.side2 + otherCone.start) || otherCone.IsWithinCone(side1 + start) ||
           otherCone.IsWithinCone(side2 + start);
}
