...
Arc::Arc(Vector2 center, double radius, double angleStart, double angleEnd) : Arc(center, radius, radius, angleStart, angleEnd) {}
...
bool Arc::isCircle() const { return length == width && angleStart == 0 && angleEnd == ARC_MAX; }

bool Arc::isPartialCircle() const { return length == width; }
...
std::pair<std::optional<Vector2>, std::optional<Vector2>> Arc::intersectionWithLine(Vector2 lineStart, Vector2 lineEnd) const {
    ...
    if (isPartialCircle()) {
        ...
        if (discr >= 0) {
            ...
            if (fabsl(discr) < .0001) {
                ...
                // two intersections
                second = checkAndDenormalize(Vector2((d * diff.y - sign * diff.x * sqrtl(discr)) / (dr * dr), (-d * diff.x - fabsl(diff.y) * sqrtl(discr)) / (dr * dr)));
            }
            return {first, second};
        } else {
            // no intersections
...
std::optional<Vector2> Arc::arcPointTowards(Vector2 point) const { return arcPointTowards((point - center).angle()); }
...
std::optional<Vector2> Arc::arcPointTowards(double angle) const {
    ...
    Vector2 point((sign * length * width) / (sqrtl(width * width + length * length * tan2)), (sign * length * width) / (sqrtl(length * length + width * width / tan2)));
    return point;
}
