...
Vector2::Vector2(rtt::Angle &angle, const double &length) : x{cos(angle.getAngle()) * length}, y{sin(angle.getAngle()) * length}, epsilon{0.00001} {}

double Vector2::dot(const Vector2 &other) const { return this->x * other.x + this->y * other.y; }
...
Vector2 Vector2::scale(double scalar) const { return {x * scalar, y * scalar}; }
...
double Vector2::length2() const { return (x * x + y * y); }
...
double Vector2::cross(const Vector2 &other) const { return this->x * other.y - this->y * other.x; }

bool Vector2::operator==(const Vector2 &other) const { return fabs(this->x - other.x) < epsilon && fabs(this->y - other.y) < epsilon; }
...
Vector2 Vector2::operator+=(const Vector2 &other) { return {this->x += other.x, this->y += other.y}; }
...
std::ostream &operator<<(std::ostream &os, rtt::Vector2 const &vec) { return vec.write(os); }
